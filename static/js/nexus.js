...
function switchNexusTab(tab){
  ...
  // #nexus-main is always in flow (display:block in CSS) — never hide it.
  // #nexus-architect and #nexus-camera are position:fixed overlays — animate in, instant out.
  ['architect','camera'].forEach(function(t){
    var panel = document.getElementById('nexus-'+t);
    if(t===tab){
      ...
      panel.offsetHeight;
      panel.style.transition = 'opacity 300ms cubic-bezier(0.23,1,0.32,1),transform 300ms cubic-bezier(0.23,1,0.32,1)';
      panel.style.opacity = '1';
      panel.style.transform = 'translateY(0)';
      setTimeout(function(){ panel.style.zIndex = ''; }, 300);
    } else {
      var _p = panel;
      if(tab === 'main'){
        // fade out overlay → reveals Main underneath
        _p.style.transition = 'opacity 260ms cubic-bezier(0.23,1,0.32,1)';
        _p.style.opacity = '0';
        setTimeout(function(){
        ...
        // overlay→overlay: hold until entering overlay covers (z-index:201)
        setTimeout(function(){
          _p.style.display = 'none';
          _p.style.opacity = '';
          _p.style.transform = '';
  ...
  if(tab==='architect'){
    ...
    if(!localStorage.getItem('chibio_tour_done')){
      setTimeout(function(){
        var f = _getArchFrame();
        if(f && f.contentWindow) f.contentWindow.postMessage({ type:'archCmd', cmd:'tour' }, window.location.origin);
      }, 300);
    }
  }
...
window.addEventListener('message', function(e){
  ...
  if(e.data.type === 'setVolume'){
    var v=Math.min(20,Math.max(0,parseFloat(e.data.val)||0));
    window._lastSyncedVolume=v;
    $('#VolumeInput').val(v);
    ajax('/SetOutputTarget/Volume/0/'+v,null);
  }
});
...
function toggleTheme() {
  ...
  if(logoEl) logoEl.src = newTheme === 'light' ? '/static/logo_negro.png' : '/static/logo.png';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var logoEl = document.getElementById('topbar-logo');
  if(logoEl) logoEl.src = th === 'light' ? '/static/logo_negro.png' : '/static/logo.png';
});
...
function switchPane(n){
  ['fp','term','proto'].forEach(function(t){
    var pane = document.getElementById('cpane-'+t);
    var tab  = document.getElementById('ctab-'+t);
    tab.classList.toggle('on', t===n);
    if(t===n){
      pane.classList.remove('pane-visible');
      pane.classList.add('on');
      pane.offsetHeight; // reflow — fuerza opacity:0 antes de la transición
      pane.classList.add('pane-visible');
    } else {
      pane.classList.remove('on','pane-visible');
    }
  });
}
...
async function analyzeProtocol(){
  ...
  try {
    ...
    var html = (data.analysis || 'Sin respuesta.').split('\n')
      ...
      .map(function(p){ return '<p style="margin-bottom:8px;">' + _esc(p) + '</p>'; })
      .join('');
    res.innerHTML = html;
...
var CUSTOM_PROGRAM_CONFIG={
  ...
  C2:{label:'μ objetivo',  min:0, max:1,    step:0.01, unit:'h⁻¹'},
  C3:{label:'μ objetivo',  min:0, max:1,    step:0.01, unit:'h⁻¹'},
  C4:{label:'Punto tiempo inicial', min:0, max:9999, step:1, unit:'ciclos'},
  C5:{label:'Punto tiempo inicial', min:0, max:9999, step:1, unit:'ciclos'},
  C6:{label:'Punto tiempo inicial', min:0, max:9999, step:1, unit:'ciclos'},
  C7:{label:'μ objetivo',  min:0, max:1,    step:0.01, unit:'h⁻¹'},
  C8:{label:'Slot usuario (sin parámetro)', disabled:true}
};
function updateCustomFieldUI(){
  var prog=$('#CustomProgram1').val();
  var cfg=CUSTOM_PROGRAM_CONFIG[prog]||{};
  var inp=document.getElementById('CustomInput');
  var hint=document.getElementById('custom-field-hint');
  var lbl=document.getElementById('custom-hint-label');
  var range=document.getElementById('custom-hint-range');
  if(cfg.disabled){
    inp.disabled=true;
    inp.value='0';
    ...
    lbl.textContent=cfg.label;
    range.textContent=cfg.min+'–'+cfg.max+(cfg.unit?' '+cfg.unit:'');
  }
  validateCustomInput();
}
function validateCustomInput(){
  ...
  var cfg=CUSTOM_PROGRAM_CONFIG[prog]||{};
  var inp=document.getElementById('CustomInput');
  if(cfg.disabled){inp.classList.remove('bad');return;}
  var v=parseFloat(inp.value);
  var bad=isNaN(v)||v<cfg.min||v>cfg.max;
  inp.classList.toggle('bad',bad);
}
function toggleCustom(){
  var prog=$('#CustomProgram1').val();
  var cfg=CUSTOM_PROGRAM_CONFIG[prog]||{};
  var v=parseFloat($('#CustomInput').val());
  if(!cfg.disabled){
    if(isNaN(v)){alert('Valor inválido para '+prog);return;}
...
function toggleFPCollapse(n){
  var card=document.getElementById('fp-card-'+n);
  if(!card)return;
  var collapsed=card.classList.toggle('fp-collapsed');
  localStorage.setItem('fp-collapsed-'+n, collapsed?'1':'0');
}

document.addEventListener('DOMContentLoaded',function(){
  ['1','2','3'].forEach(function(n){
    if(localStorage.getItem('fp-collapsed-'+n)==='1'){
      var card=document.getElementById('fp-card-'+n);
      if(card)card.classList.add('fp-collapsed');
    }
  });
  // Marcar pane activo inicial como visible (sin animación)
  var activePane = document.querySelector('.cpane.on');
  if(activePane) activePane.classList.add('pane-visible');
});

/* Presets de proteína fluorescente — autocompletan Excitación/Base/Emisión.
   Valores aproximados a la opción de hardware más cercana en cada canal. */
var FP_PRESETS={
  GFP    : {excite:'LEDB',     base:'nm470', em1:'nm510', em2:'OFF'},
  YFP    : {excite:'LEDC',     base:'nm510', em1:'nm550', em2:'OFF'},
  RFP    : {excite:'LEDD',     base:'nm550', em1:'nm583', em2:'OFF'},
  mCherry: {excite:'LEDF',     base:'CLEAR', em1:'nm620', em2:'OFF'},
  Cy5    : {excite:'LASER650', base:'CLEAR', em1:'nm670', em2:'OFF'},
};
function applyFPPreset(n){
  var key=$('#FPProtein'+n).val();
  if(!key)return; // "Personalizado" — no tocar selects
  var p=FP_PRESETS[key];
  if(!p)return;
  $('#FPExcite'+n).val(p.excite);
  $('#FPBase'+n).val(p.base);
  $('#FPEmit'+n+'A').val(p.em1);
  $('#FPEmit'+n+'B').val(p.em2);
}
function fpManualEdit(n){$('#FPProtein'+n).val('');} // edición manual rompe preset
...
function _updateDataInner(data){
  ...
  var _eS=document.getElementById('ExperimentStart');if(_eS)_eS.disabled=running;
  var _eR=document.getElementById('ExperimentReset');if(_eR)_eR.disabled=running;
  var _eP=document.getElementById('ExperimentStop');if(_eP)_eP.disabled=!running;
  ...
  if(window._lastSyncedVolume!==data.Volume.target){window._lastSyncedVolume=data.Volume.target;archCmd('setVolume',data.Volume.target);}
  ...
  ['FP1','FP2','FP3'].forEach(function(fp,fi){
    ...
    if(sw){
      var on=data[fp].ON===1;
      sw.style.borderColor=on?'var(--gr)':'';
      sw.style.color=on?'var(--gr)':'';
...
function startMainTour(){
  ...
  var mto = document.getElementById('main-tour-overlay');
  mto.classList.add('active'); mto.offsetHeight; mto.classList.add('visible');
  _renderMainTourStep();
}
...
function endMainTour(){
  ...
  document.getElementById('main-tour-overlay').classList.remove('active','visible');
