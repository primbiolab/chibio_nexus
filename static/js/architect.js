...
let globalInitVol = 20.0; // ml — sincronizado con Volume del reactor en Nexus, máx 20ml (capacidad tubo de vidrio)
let rawPythonCode = "";
...
// ── Modals & Settings ─────────────────────────────────────
function _openModal(el){ el.classList.add('open'); el.offsetHeight; el.classList.add('visible'); }
function _closeModal(el){ el.classList.remove('open','visible'); }
...
function openPumpModal() {
    ...
    _openModal(document.getElementById('modal-pump'));
}
function closePumpModal(e){ if(e.target===document.getElementById('modal-pump')) _closeModal(document.getElementById('modal-pump')); }
function savePumpModal() {
    ...
    globalInitVol = Math.min(20, Math.max(0, parseFloat(document.getElementById('p_init_vol').value) || 0));
    _closeModal(document.getElementById('modal-pump'));
    if(window.parent !== window) window.parent.postMessage({ type:'setVolume', val:globalInitVol }, window.location.origin);
    refresh(); toast('Caudales y volumen inicial actualizados', 'ok');
}

// Cloud config — ahora solo abre el modal informativo
function openCloudModal(){
  _openModal(document.getElementById('modal-cloud'));
}
function saveCloudConfig(){
  _closeModal(document.getElementById('modal-cloud'));
}
function openSafety(){  cs1=false; cs2=false; cs3=false;['cs1','cs2','cs3'].forEach(id=>document.getElementById(id).classList.remove('done'));
  ...
  _openModal(document.getElementById('modal-safety'));
}
function toggleCS(n){
...
function closeSafetyBtn(ok){
  _closeModal(document.getElementById('modal-safety'));
  if(ok){
    safetyOk=true;
...
function checkSingletons() {
  ...
  const lockedTypes = [
    'thermostat','ramp_temp','led','uv','laser','pump', // Hardware
...
function _animateNewBlock(node){
  var el=document.getElementById('cb-'+node.id);
  if(el){el.classList.add('cb-new');setTimeout(function(){el.classList.remove('cb-new');},800);}
}
function canvasDrop(e){
  e.preventDefault(); document.getElementById('dropzone').classList.remove('vis');
  if(dragFromPalette){ var n=mkNode(dragFromPalette); AST.push(n); refresh(); _animateNewBlock(n); dragFromPalette=null; }
}
...
function addBlock(type, arr){ 
    ...
    const node=mkNode(type); (arr||AST).push(node); refresh(); _animateNewBlock(node);
}
function refresh(){ 
  _historySave();
    ...
    if(dragFromCanvas && dragFromCanvas.id !== node.id){
      ...
      if(r){
        ...
        if(isFixedBlock(node.type) && !SINGLETON_TYPES.includes(dragFromPalette)){
          ...
          _newNode=mkNode(dragFromPalette); r.arr.splice(afterSingletons, 0, _newNode);
        } else {
          _newNode=mkNode(dragFromPalette); r.arr.splice(insertIdx, 0, _newNode);
        }
      }
      refresh(); if(_newNode) _animateNewBlock(_newNode); dragFromPalette=null;
    }
  });
...
function openPicker(arr){
  ...
  _openModal(document.getElementById('modal-picker'));
}
function closePicker(e){ if(e.target===document.getElementById('modal-picker')) closePickerBtn(); }
function closePickerBtn(){ _closeModal(document.getElementById('modal-picker')); pickerTarget=null; }
...
function calcVol(arr){
  ...
  for(const n of (arr||AST)){
    ...
    if(n.children){ const s=calcVol(n.children); const c=n.count!=null?n.count:1; inV+=s.inV*c; outV+=s.outV*c; }
  } return {inV,outV};
}
function updateVol(){
  const {inV,outV}=calcVol(AST); const net=Math.max(0,globalInitVol+inV-outV); const pct=Math.min(100,(net/20)*100);
  document.getElementById('vol-bar').style.width=pct+'%'; document.getElementById('vol-bar').classList.toggle('danger',net>20);
  document.getElementById('vol-val').textContent=net.toFixed(2)+' / 20 ml'; document.getElementById('vol-val').style.color=net>20?'var(--danger)':'';
}
...
function validate(nodes,errs,warns,M){
  ...
  const {inV,outV}=calcVol(nodes); if(inV-outV>20 && nodes===AST) errs.push('[Bombas] Volumen de entrada neto superará los 20ml de capacidad del tubo de vidrio. Inundación inminente.');
...
const TOUR_STEPS =[
  ...
  { sel:'.vol-bar-outer', pos:'top',
    ...
    desc:'Lleva la cuenta del volumen neto que inyectarán las bombas a lo largo del protocolo. Si superas los 20 ml el compilador bloqueará el experimento.' },
...
function startTour(){
  ...
  var to = document.getElementById('tour-overlay');
  to.classList.add('active'); to.offsetHeight; to.classList.add('visible');
  renderTourStep();
}
...
function endTour(){
  ...
  document.getElementById('tour-overlay').classList.remove('active','visible');
...
// Auto-launch removido: el tour de Architect solo debe iniciar cuando esa pestaña
// es visible (ver nexus.js switchTab), nunca mientras el iframe carga oculto en background.
// De lo contrario tourOverlay:active se envía al padre sin que el usuario lo vea,
// dejando el topbar dim hasta que entra manualmente a Architect.
...
function addMsg(type, icon, html){ const ce=document.getElementById('console-wrap').querySelector('.console-empty-msg'); if(ce) ce.remove(); const d=document.createElement('div'); d.className='cmsg '+type; d.innerHTML=icon+' '; const sp=document.createElement('span'); sp.textContent=html; d.appendChild(sp); document.getElementById('console-wrap').appendChild(d); }
function switchTab(name){
  ['console','code'].forEach(t=>{
    var pane = document.getElementById('rpane-'+t);
    var tab  = document.getElementById('rtab-'+t);
...
window.addEventListener('message', function(e){
  ...
  if(e.data.type === 'setTheme'){
    ...
    var archLogo = document.getElementById('arch-logo');
    if(archLogo) archLogo.src = e.data.theme === 'light' ? '/static/logo_negro.png' : '/static/logo.png';
  }
  if(e.data.type === 'archCmd'){
    switch(e.data.cmd){
      ...
      case 'setVolume':
        globalInitVol = Math.min(20, Math.max(0, parseFloat(e.data.val) || 0));
        var _pInit = document.getElementById('p_init_vol');
        if(_pInit) _pInit.value = globalInitVol;
        updateVol();
        break;
      case 'importData':
        try {
...
window.addEventListener('load', function(){
  setTimeout(_emitState, 500);
  var activeRpane = document.querySelector('.rpane.on');
  if(activeRpane) activeRpane.classList.add('pane-visible');
});
