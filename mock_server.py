...
# ── Configuración ────────────────────────────────────────────────
# Cuando corre empacado por PyInstaller (--onefile), los datos extra
# (templates/, static/) se extraen en sys._MEIPASS, no junto al .exe.
import sys
...
# Overrides mutables por reactor — escritos por Set* endpoints, mergeados en _build_sysdata
_REACTOR_MUT = {dev: {} for dev in ('M0', 'M1', 'M2')}


def _merge_mut(base: dict, device_id: str) -> dict:
    for key, overrides in _REACTOR_MUT.get(device_id, {}).items():
        if key in base and isinstance(base[key], dict):
            base[key].update(overrides)
    return base

# ── Protocolo de experimento simulado ─────────────────────────────
# Mantener 37°C y OD 0.3 por 4h, luego OD 0.5 por otras 4h (ciclo se repite).
# TIME_SCALE comprime el tiempo simulado para verlo avanzar en minutos reales
# en vez de horas (60x → ciclo completo de 8h dura 8 min reales).
PROTOCOL_TEMP           = 37.0
PROTOCOL_OD_PHASE1      = 0.3
PROTOCOL_OD_PHASE2      = 0.5
PROTOCOL_PHASE_DURATION = 4 * 3600   # segundos simulados (4h)
PROTOCOL_CYCLE          = 2 * PROTOCOL_PHASE_DURATION
TIME_SCALE              = 60.0

def _protocol_od_target(t_real: float) -> float:
    """OD objetivo del protocolo según tiempo real transcurrido (comprimido)."""
    if t_real <= 0:
        return PROTOCOL_OD_PHASE1
    t_sim = t_real * TIME_SCALE
    t_in_cycle = t_sim % PROTOCOL_CYCLE
    return PROTOCOL_OD_PHASE1 if t_in_cycle < PROTOCOL_PHASE_DURATION else PROTOCOL_OD_PHASE2
...
def _make_od_record(length: int, t_now: float, amp: float, freq: float, noise: float = 0.02, spacing: float = 60.0) -> str:
    """Serie de OD que sigue el target del protocolo (escalón en la fase de 4h)."""
    vals = []
    for i in range(length):
        sample_t = t_now - (length - 1 - i) * spacing
        target = _protocol_od_target(sample_t)
        v = target + amp * math.sin(freq * i) + random.gauss(0, noise)
        vals.append(round(max(0.0, v), 4))
    return ','.join(str(v) for v in vals)

def _make_od_target_record(length: int, t_now: float, spacing: float = 60.0) -> str:
    """Serie del target de OD — refleja el escalón 0.3 → 0.5 a las 4h."""
    vals = []
    for i in range(length):
        sample_t = t_now - (length - 1 - i) * spacing
        vals.append(str(_protocol_od_target(sample_t)))
    return ','.join(vals)
...
def _build_sysdata(device_id: str) -> dict:
    ...
    od_target_now = _protocol_od_target(t)
    od_now   = od_target_now + cfg['od_amp'] * math.sin(cfg['od_freq'] * t) + random.gauss(0, 0.005)
    od_now   = max(0.0, od_now)
    temp_now = PROTOCOL_TEMP + random.gauss(0, 0.08)
    running  = cfg['running']

    # Records de series temporales
    od_rec   = _make_od_record(n, t, cfg['od_amp'], cfg['od_freq'])
    od_tgt_rec = _make_od_target_record(n, t)
    mu_rec   = _make_record(n, 0.35,           0.10,           cfg['od_freq'] * 1.3, 0.015)
    t_ir_rec = _make_record(n, PROTOCOL_TEMP,  0.0,           0.0,  0.08)
    t_tgt_rec= ','.join([str(PROTOCOL_TEMP)] * n)
    t_int_rec= _make_record(n, PROTOCOL_TEMP - 0.5, 0.2,   0.025)
    t_ext_rec= _make_record(n, PROTOCOL_TEMP - 1.2, 0.3,   0.02)
    ...
    result = {
        ...
        'OD': {
            ...
            'target'       : od_target_now,
            'default'      : 0.5,
            'max'          : 10,
            'min'          : 0,
            'record'       : od_rec,
            'targetrecord' : od_tgt_rec,
            'Measuring'    : 0,
            'ON'           : 1 if running else 0,
            'Integral'     : 0.0,
            'Integral2'    : 0.0,
        ...
        'Thermostat': {
            'default'   : PROTOCOL_TEMP,
            'target'    : PROTOCOL_TEMP,
            'max'       : 50.0,
            'min'       : 0.0,
            'ON'        : 1 if running else 0,
            'record'    : t_tgt_rec,
...
def changeDevice(M):
    ...
    if M not in ['M0', 'M1', 'M2']:
        return jsonify({'error': 'Dispositivo no presente'}), 400
    _ui_device = M
...
def experiment(on, M):
    global _REACTOR_CFG, _REACTOR_MUT, _ui_device
    _REACTOR_CFG[_ui_device]['running'] = bool(on)
    # Limpiar overrides de ON para que los actuadores reflejen el nuevo estado del experimento
    for key in list(_REACTOR_MUT.get(_ui_device, {}).keys()):
        _REACTOR_MUT[_ui_device][key].pop('ON', None)
    print(f'[MOCK] Experimento {"INICIADO" if on else "DETENIDO"} en {_ui_device}')
    return ('', 204)

@app.route('/ExperimentReset', methods=['POST'])
def experimentReset():
    global _start_time, _REACTOR_CFG, _REACTOR_MUT, _ui_device
    _start_time = time.time()
    _REACTOR_CFG[_ui_device]['running'] = False
    _REACTOR_CFG[_ui_device]['fsm']     = 0.0
    _REACTOR_MUT[_ui_device] = {}
...
# Endpoints de control — mutan _REACTOR_MUT y responden 204
@app.route('/SetOutputTarget/<param>/<a>/<val>', methods=['POST'])
def setOutputTarget(param, a, val):
    global _REACTOR_MUT, _REACTOR_CFG
    try:
        fval = float(val)
    except ValueError:
        return ('', 204)
    mut = _REACTOR_MUT.setdefault(_ui_device, {}).setdefault(param, {})
    mut['target'] = fval
    # Propagar a cfg para que la simulación siga el nuevo target
    if param == 'OD':
        _REACTOR_CFG[_ui_device]['od_base'] = fval
    elif param == 'Thermostat':
        _REACTOR_CFG[_ui_device]['temp_base'] = fval
    elif param == 'Stir':
        _REACTOR_CFG[_ui_device]['stir'] = fval
    elif param == 'Pump1':
        _REACTOR_CFG[_ui_device]['pump1'] = fval
    print(f'[MOCK] SetOutputTarget {param}={fval} en {_ui_device}')
    return ('', 204)

@app.route('/SetOutputOn/<param>/<a>/<b>', methods=['POST'])
def setOutputOn(param, a, b):
    mut = _REACTOR_MUT.setdefault(_ui_device, {}).setdefault(param, {})
    if 'ON' not in mut:
        base_on = _build_sysdata(_ui_device).get(param, {}).get('ON', 0)
        mut['ON'] = 0 if base_on else 1
    else:
        mut['ON'] = 0 if mut['ON'] else 1
    print(f'[MOCK] SetOutputOn {param} -> ON={mut["ON"]} en {_ui_device}')
    return ('', 204)

@app.route('/SetCustom/<prog>/<state>', methods=['POST'])
def setCustom(prog, state):
    mut = _REACTOR_MUT.setdefault(_ui_device, {}).setdefault('Custom', {})
    cfg_running = _REACTOR_CFG[_ui_device]['running']
    currently_on = mut.get('ON', 1 if cfg_running else 0)
    if currently_on:
        mut['ON'] = 0
    else:
        mut['ON'] = 1
        mut['Program'] = prog
        try:
            mut['Status'] = float(state)
        except ValueError:
            mut['Status'] = 0.0
    print(f'[MOCK] SetCustom {prog} state={state} -> ON={mut["ON"]} en {_ui_device}')
    return ('', 204)

# Endpoints sin efecto en estado — solo log
_GENERIC_ENDPOINTS = [
    '/MeasureOD/<M>',
...
def analyzeProtocol():
    mock_code = (
        ...
        "    if data['Experiment']['cycles'] < 240:\n"
        "        data['OD']['target'] = 0.3\n"
        "    else:\n"
        "        data['OD']['target'] = 0.5\n"
    )
    mock_analysis = (
        "El protocolo mantiene la temperatura en 37°C durante todo el experimento. "
        "Durante las primeras 4 horas (240 ciclos de 1 minuto) mantiene el OD objetivo "
        "en 0.3; transcurrido ese tiempo, cambia el OD objetivo a 0.5 y lo mantiene "
        "por otras 4 horas."
    )
